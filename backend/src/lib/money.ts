...
export function toDecimal(value: MoneyInput): Decimal {
  ...
  if (!d.isFinite()) throw new Error(`Not a finite amount: ${String(value)}`);
...
export const isPositive = (v: MoneyInput): boolean => toDecimal(v).greaterThan(0);
export const isNonNegative = (v: MoneyInput): boolean => toDecimal(v).greaterThanOrEqualTo(0);

/** True for a positive amount that SOL can actually represent. */
export function isValidAmount(value: unknown): boolean {
  if (value === null || value === undefined) return false;
  try {
    const d = toDecimal(value as MoneyInput);
...
export function toLamports(value: MoneyInput): bigint {
  const d = toDecimal(value);
  return BigInt(d.times(LAMPORTS_PER_SOL.toString()).toFixed(0));
}

export function fromLamports(lamports: bigint): Decimal {
...
export function applyFeeBps(
  amount: MoneyInput,
  bps: number,
): { fee: Decimal; remainder: Decimal } {
  const total = toLamports(amount);
  const fee = (total * BigInt(bps)) / 10_000n; // BigInt division truncates
 ...
 */
export function splitByWeight(total: MoneyInput, weights: number[]): Decimal[] {
  ...
  if (weightSum <= 0) throw new Error('splitByWeight requires weights summing above zero');
