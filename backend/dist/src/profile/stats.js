...
const clampPositive = (v) => toAmountString(v.startsWith('-') ? '0' : v);
const clampNegative = (v) => toAmountString(v.startsWith('-') ? v : '0');
/**
 ...
 */
async function settledAggregate(userId) {
...
async function ledgerTotals(userId) {
    const rows = await prisma.$queryRaw `
 ...
 */
export async function lifetimeStats(userId) {
...
export async function perGameStats(userId) {
    const rows = await prisma.$queryRaw `
 ...
 */
export async function dailyNet(userId, days = 30) {
    const rows = await prisma.$queryRaw `
