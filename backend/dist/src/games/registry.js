...
export function registerGame(mod) {
    ...
    if (byId.has(id))
        throw new Error(`Duplicate game id: ${id}`);
    byId.set(id, mod);
...
export function buildGamesRouter() {
    ...
    for (const mod of byId.values()) {
        if (mod.router)
            router.use(`/${mod.manifest.id}`, mod.router);
    }
    return router;
}
/** Lets every game attach its handlers to a freshly authenticated socket. */
export function registerGameSockets(namespace, socket) {
    for (const mod of byId.values()) {
...
export async function shutdownGames() {
    for (const mod of byId.values()) {
        try {
            await mod.shutdown?.();
        }
