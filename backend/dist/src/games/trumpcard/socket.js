...
import { CARD_LIMITS, ROUND_REVEAL_DELAY_MS, STAT_CHOICE_TIMEOUT_MS, calculatePayoutWeights, checkMatchEnd, createInitialState, decrementLife, getCardById, getNextLeader, markDisconnected, markReconnected, rankFinalStandings, resolveRound, } from './engine.js';
 ...
 * payloads small.
 */
function buildStateSnapshot(match, forUserId) {
...
function broadcastSnapshots(match, extra) {
    const io = getIoRef();
    if (!io)
        return;
...
// --- Turn management --------------------------------------------------------
function startLeaderTimer(match, leaderId) {
 ...
 */
function applyLifeLoss(match, userId, cause) {
    ...
    if (eliminated) {
        ...
        guard('forfeit on lives elimination', escrow.forfeitPlayer(match.matchId, userId, 0), { matchId: match.matchId, userId });
    }
...
async function settleMatch(match) {
    clearTimeouts(match);
    for (const timer of Object.values(match.timers.disconnect))
        clearTimeout(timer);
    ...
    const result = await escrow.settleMatch(match.matchId, winnerIds.length > 0 ? winnerIds : [match.playerIds[0]], weights.length > 0 ? weights : [1], { result: { rankings: resultRankings, seatCount: match.seatCount } });
...
export function registerTrumpcardSocket(namespace, socket) {
    ...
    socket.on(TRUMPCARD_EVENTS.CREATE_MATCH, async (data) => {
        try {
            ...
            const maxCards = CARD_LIMITS[seatCount];
    ...
    socket.on(TRUMPCARD_EVENTS.JOIN_MATCH, async (data) => {
        try {
            let matchId;
            let usedRoomCode;
            if (data.roomCode) {
            ...
            const cardsPerPlayer = gs.cardsPerPlayer ?? CARD_LIMITS[seatCount] ?? 26;
    ...
    socket.on(TRUMPCARD_EVENTS.CHOOSE_STAT, (data) => {
        try {
            ...
            const { state: newState, comparison, winnerId, tiedIds, poolClaimedBy, newlyEliminated } = resolveRound(match.state, statKey);
    ...
    socket.on(TRUMPCARD_EVENTS.LEAVE_LOBBY, async (data) => {
        try {
    ...
    socket.on('disconnect', () => {
        try {
            ...
            if (!match.state.activePlayerIds.includes(userId))
                return;
            ...
            if (match.timers.disconnect[userId])
                return;
