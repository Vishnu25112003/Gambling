...
## Last Updated
- 2026-08-18 — Anti-Sybil payout gate (`payoutEligibility.ts`): deposit + wagering thresholds, gating the payout rather than the binding. `/me` now returns `payoutRequirements` and a per-friend `unlocked` flag; the page renders `getting started` as a third state. 10 new tests (29 total). Remaining Sybil gap — commission size on a high-multiplier win — recorded in Open Questions.
