...
function serialize(userId, task) {
...
async function performWithdrawal(userId, amount) {
    ...
    if (sendAmount.lessThanOrEqualTo(0)) {
        throw badRequest(`Withdrawal is too small to cover the ${toAmountString(networkFee)} SOL network fee.`);
    }
    /**
    ...
    try {
        ...
        await prisma.$transaction(async (tx) => {
            ...
            await tx.ledgerEntry.update({
                ...
                data: {
                    status: 'failed',
                    note: `Withdrawal failed and was reversed: ${err.message}`,
...
async function estimateFee(connection, from, to) {
    try {
        const { blockhash } = await connection.getLatestBlockhash(commitment);
        const tx = new Transaction({ feePayer: from, recentBlockhash: blockhash }).add(SystemProgram.transfer({ fromPubkey: from, toPubkey: to, lamports: 1 }));
