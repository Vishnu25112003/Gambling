    ...
    return (
      <GameLobby
        ...
        matches={matches.map((m) => ({
          matchId: m.matchId,
          hostName: m.hostName,
          meta: `${m.seatCount} players · ${m.cardsPerPlayer} cards each · ${m.durationMinutes} min · ${
            m.betMode === 'fixed' ? 'Fixed' : 'Free'
          } bet${m.betMode === 'free' && m.minBet ? ` · min ${formatSol(m.minBet)}` : ''} · ${formatSol(m.stake)} SOL`,
        }))}
        onCreate={() => setPage('create')}
        onJoinByCode={() => setPage('join_code')}
        onJoinMatch={handleJoinRandom}
        onRefresh={() => socketRef.current?.emit(TRUMPCARD.LIST_MATCHES)}
      />
    );
  }
