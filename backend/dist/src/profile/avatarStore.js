...
export const avatarUpload = (req, res, next) => {
    upload.single(AVATAR_FIELD)(req, res, (err) => {
        if (!err) {
            next();
            return;
        }
...
export function avatarFilename(userId) {
    return `${userId}.webp`;
}
/**
 ...
 */
export async function saveAvatar(userId, buffer, previousUrl) {
    ...
    try {
        ...
        log.warn('avatar re-encode failed', { userId, reason: err.message });
...
export async function removeAvatar(userId) {
    try {
        ...
        if (err.code !== 'ENOENT') {
            log.warn('avatar unlink failed', { userId, reason: err.message });
        }
    }
}
/** Read `?v=N` off the previous URL and step it. Starts at 1. */
function nextVersion(previousUrl) {
    if (!previousUrl)
        return 1;
...
//# sourceMappingURL=avatarStore.js.map