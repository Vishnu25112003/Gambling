 ...
 */
export async function bindReferral(referredUserId, rawCode) {
    const code = normaliseCode(rawCode);
    if (!isPlausibleCode(code))
        throw notFound('That invite code does not exist.');
 ...
 */
export function referrerLabel(user) {
    return user.username ?? shortAddress(user.walletAddress);
}
//# sourceMappingURL=bindReferral.js.map