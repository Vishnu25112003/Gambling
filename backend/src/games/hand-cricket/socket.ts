...
import {
  ...
  currentInnings,
  PICK_TIMEOUT_MS,
  VALID_BALLS_PER_INNINGS,
} from './engine.js';
import type { HandCricketState } from './types.js';
...
function startBall(match: ActiveMatch): void {
  ...
  match.timers.ball = setTimeout(
    () => guardSync('ball timeout', () => handleBallTimeout(match), { matchId: match.matchId }),
    PICK_TIMEOUT_MS,
  );
}

function handleBallTimeout(match: ActiveMatch): void {
  const innings = currentInnings(match.state);
  const pending = match.state.pendingBall;
  if (!innings || !pending) return;
 ...
 */
function afterBallResolved(match: ActiveMatch): void {
...
async function settleMatch(
  ...
  for (const socketId of Object.values(match.socketIds)) socketToMatch.delete(socketId);
...
function handleDualUnreachable(match: ActiveMatch): void {
  clearTimeouts(match);
...
async function beginMatch(
  ...
  for (const sid of Object.values(newActiveMatch.socketIds)) socketToMatch.set(sid, matchId);
...
export function registerHandCricketSocket(namespace: Namespace, socket: Socket): void {
  ...
  socket.on(HC_EVENTS.JOIN_MATCH, async (data: { matchId?: string; roomCode?: string; stake?: number }) => {
    try {
      ...
      const gs = (dbMatch.gameState ?? {}) as { ballsPerInnings?: number; betMode?: string; minBet?: number };
      ...
      await beginMatch(
        matchId,
        playerIds,
        { ballsPerInnings, betMode, minBet: gs.minBet ?? null, stake: stakeAmount },
        socketIds,
      );
  ...
  socket.on(HC_EVENTS.PICK_NUMBER, async (data: { pick?: number }) => {
    try {
      ...
      if (!bothPicksIn(match.state)) return;
      ...
      broadcastToMatch(match, HC_EVENTS.BALL_RESULT, {
        ...
        batterId: innings.batterId,
        bowlerId: innings.bowlerId,
        totalRuns: match.state.innings[match.state.currentInningsIndex as number]?.runs,
      });
  ...
  // --- REMATCH_REQUEST ---
  socket.on(HC_EVENTS.REMATCH_REQUEST, async (data: { matchId?: string }) => {
    try {
