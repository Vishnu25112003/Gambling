...
import { isUsernameAvailable, isReservedUsername, isValidUsername, normaliseUsername, } from './username.js';
 ...
 * which handles exist.
 */
profileRouter.get('/username/check', requireAuth, asyncHandler(async (req, res) => {
 ...
 */
profileRouter.get('/:handle', optionalAuth, asyncHandler(async (req, res) => {
...
profileRouter.get('/:handle/history', optionalAuth, asyncHandler(async (req, res) => {
    const userId = await resolveHandle(String(req.params.handle ?? ''));
    const { page, limit } = parsePaging(req.query);
