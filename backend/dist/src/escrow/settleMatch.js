 ...
 */
export async function settleMatch(matchId, winners, weights, options = {}) {
    ...
    `;
        ...
        const pot = toDecimal(match.pot);
        const isPooled = match.mode === 'pooled';
        let feeCollected = new Decimal(0);
        const payoutByUser = new Map();
        const addPayout = (uid, amount) => payoutByUser.set(uid, (payoutByUser.get(uid) ?? new Decimal(0)).plus(amount));
        if (isPooled) {
            ...
            if (winners.length > 0 && totalWeight > 0) {
                ...
                match.participants.forEach((p) => addPayout(p.userId, toDecimal(p.lockedAmount).plus(toDecimal(p.forfeitedAmount))));
            }
        }
        else {
            ...
            let parsed;
            try {
                parsed = weights.map((w) => toDecimal(w));
            }
            catch (err) {
                throw badRequest(`For solo_vs_house matches, weights are absolute SOL payouts: ${err.message}`);
            }
            ...
            const totalPaid = [...payoutByUser.values()].reduce((a, b) => a.plus(b), new Decimal(0));
        ...
        for (const participant of match.participants) {
            ...
            const totalStake = stillLocked.plus(toDecimal(participant.forfeitedAmount));
