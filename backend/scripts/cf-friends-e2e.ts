...
function fail(label: string): never { console.log(`  \x1b[31m✗ ${label}\x1b[0m`); process.exit(1); }

/** Resolve once `event` arrives, or reject after `ms`. */
function waitFor<T>(s: Socket, event: string, ms = 8000): Promise<T> {
...
async function main() {
  ...
  for (const [name, s] of [['host', hs], ['friend', fs]] as const) {
    s.on('cf:error', (e: { message: string }) => console.log(`  \x1b[33m! ${name} cf:error: ${e.message}\x1b[0m`));
  }
  ...
  if (!c.matchId) fail('cf:created carried no matchId');
  ok(`match created: ${c.matchId}`);
  if (!c.roomCode) fail('cf:created carried NO roomCode — Friends Play is broken');
  if (!/^[A-HJ-NP-Z2-9]{6}$/.test(c.roomCode)) fail(`roomCode "${c.roomCode}" is not a valid 6-char code`);
  ok(`room code generated: ${c.roomCode}`);

  // --- Step 2: a Friends match must NOT appear in the public lobby ---
  const listed = waitFor<{ matches: { matchId: string }[] }>(os, 'cf:matches');
  os.emit('cf:list', { gameType: 'coin-flip' });
  const l = await listed;
  if (l.matches.some((m) => m.matchId === c.matchId)) fail('private Friends match leaked into the public lobby list');
  ok('private match is hidden from the public lobby');

  // --- Step 3: a wrong code is rejected, and does NOT burn the real one ---
  const badJoin = waitFor<{ message: string }>(fs, 'cf:error');
  fs.emit('cf:join', { roomCode: 'ZZZZZZ' });
  const bad = await badJoin;
  if (!/invalid room code/i.test(bad.message)) fail(`unexpected error for a bad code: ${bad.message}`);
  ok('invalid room code rejected');

  // --- Step 4: the friend joins with the real code; BOTH sides must start ---
  const hostRound = waitFor<Record<string, unknown>>(hs, 'cf:round:start');
  const friendRound = waitFor<Record<string, unknown>>(fs, 'cf:round:start');
  ...
  if (hr.spinnerId !== fr.spinnerId) fail('the two clients disagree on who spins');
  const spinnerIsAPlayer = hr.spinnerId === host.id || hr.spinnerId === friend.id;
  if (!spinnerIsAPlayer) fail('spinner is not one of the two players');
  ...
  for (const b of balances) {
    if (Number(b.lockedBalance) !== STAKE) fail(`stake not locked for ${b.id} (locked=${b.lockedBalance})`);
  }
  ok(`both stakes locked (${STAKE} SOL each)`);

  // --- Step 6: the code is retired once the room is full ---
  const reuse = waitFor<{ message: string }>(os, 'cf:error');
  os.emit('cf:join', { roomCode: c.roomCode });
  const r = await reuse;
  if (!/invalid room code/i.test(r.message)) fail(`a used room code was not retired: got "${r.message}"`);
  ok('used room code is retired');

  console.log('\n\x1b[32mFriends Play flow works end to end.\x1b[0m\n');

  hs.disconnect(); fs.disconnect(); os.disconnect();
