...
import {
  ...
  rankFinalStandings,
  resolveRound,
} from '../src/games/trumpcard/engine.js';
import { STAT_KEYS } from '../src/games/trumpcard/types.js';
import type { TrumpcardState } from '../src/games/trumpcard/types.js';
...
function makeState(overrides: Partial<TrumpcardState> & { hands: Record<string, string[]> }): TrumpcardState {
...
describe('trumpcard engine — dealing', () => {
  it('deals the requested cards per player for each seat count and discards the rest', () => {
    for (const seatCount of [2, 3, 4] as const) {
      ...
      for (const id of ids) expect(hands[id]).toHaveLength(max);
...
describe('trumpcard engine — round resolution', () => {
  ...
  it('a tie pools every active players compared card (not just the tied ones), and the pool is claimed by the next decisive winner', () => {
    ...
    for (const p of others) expect(r2.state.hands[p]).toEqual([]);
...
describe('trumpcard engine — match end and ranking', () => {
  ...
  it('matches the Ludo/Trumpcard seat-count-scaled payout table', () => {
    ...
    expect(
      calculatePayoutWeights([{ playerId: A, rank: 1 }, { playerId: B, rank: 2 }, { playerId: C, rank: 3 }], 4),
    ).toEqual([{ userId: A, weight: 50 }, { userId: B, weight: 30 }, { userId: C, weight: 20 }]);
  });
