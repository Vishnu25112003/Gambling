...
import { isReservedUsername, isValidUsername, normaliseUsername, parseUsername, } from '../src/profile/username.js';
...
async function playPooled(a, b, stake, winner, gameType = 'test') {
...
describe('usernames', () => {
    ...
    it('is unique in the DATABASE, not merely in application code', async () => {
        ...
        await expect(db().user.update({ where: { id: bob.id }, data: { username: 'taken' } })).rejects.toThrow();
    });
    it('rejects a malformed username at the database level too', async () => {
        ...
        await expect(db().user.update({ where: { id: user.id }, data: { username: 'Bad-Name!' } })).rejects.toThrow();
    });
