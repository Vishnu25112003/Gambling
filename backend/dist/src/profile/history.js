...
function classify(row) {
    if (row.matchstatus === 'refunded' || row.participantstatus === 'refunded')
        return 'refunded';
    if (row.participantstatus === 'forfeited')
        return 'forfeited';
    if (row.matchstatus === 'open')
        return 'open';
    ...
    if (row.net.startsWith('-'))
        return 'lost';
    if (Number.parseFloat(row.net) === 0)
        return 'draw';
    return 'won';
}
/**
 ...
 */
export async function matchHistory(userId, page = 1, limit = HISTORY_DEFAULT_LIMIT) {
