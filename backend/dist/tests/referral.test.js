...
async function playPooled(a, b, stake, winner) {
...
describe('bindReferral', () => {
    ...
    it('blocks self-referral in the database even if application logic is bypassed', async () => {
        const alice = await makeUser();
        await expect(db().referral.create({ data: { referrerId: alice.id, referredUserId: alice.id } })).rejects.toThrow();
    });
