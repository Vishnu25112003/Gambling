 ...
 * drift apart. */
function publicMatchesFor(viewerUserId) {
 ...
 */
function broadcastMatchesList() {
    const io = getIoRef();
    if (!io)
        return;
    for (const sock of io.sockets.values()) {
        if (!sock.rooms.has(LOBBY_ROOM))
            continue;
        const viewerUserId = sock.data.userId;
...
// --- Timer management -------------------------------------------------------
function startSpinTimer(match, spinnerId) {
...
function startCallTimer(match, callerId) {
    match.timers.call = setTimeout(() => guardSync('call timeout', () => {
        ...
        const { state: newState, record } = resolveCall(match.state, null, // no call
...
async function handleRoundEnd(match, roundWinnerId) {
    ...
    const spinnerId = Object.entries(match.state.seats).find(([, seat]) => seat === 'spinner')?.[0];
    if (spinnerId) {
...
export function registerCoinFlipSocket(namespace, socket) {
    ...
    socket.on(CF_EVENTS.JOIN_MATCH, async (data) => {
        try {
            let matchId;
            let usedRoomCode;
            ...
            void socket.leave(LOBBY_ROOM);
            if (hostSocketId)
                void getIoRef()?.sockets.get(hostSocketId)?.leave(LOBBY_ROOM);
            ...
            await beginMatch(matchId, playerIds, { rounds: roundCount, betMode, minBet: gs.minBet ?? null, stake: hostStakeAmount }, socketIds);
    ...
    socket.on(CF_EVENTS.CALL, async (data) => {
        try {
            const matchId = socketToMatch.get(socket.id);
            if (!matchId) {
                socket.emit(CF_EVENTS.ERROR, { message: 'Not in a match' });
            ...
            // Resolve the round
            const { state: newState, record } = resolveCall(match.state, call, result, spinnerId, userId);
            match.state = newState;
            match.roundRecords.push(record);
            ...
            setTimeout(() => guardSync('reveal', () => {
                ...
                guard('round end', handleRoundEnd(match, record.cause === 'correct_call' ? userId : spinnerId), { matchId: match.matchId });
