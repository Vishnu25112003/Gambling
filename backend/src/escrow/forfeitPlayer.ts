...
interface PendingForfeit {
  ...
  resolve: (result: ForfeitResult) => void;
 ...
 */
const pending = new Map<string, PendingForfeit>();

const key = (matchId: Id, userId: Id) => `${matchId}:${userId}`;
...
export function forfeitPlayer(
  ...
  return new Promise<ForfeitResult>((resolve, reject) => {
 ...
 */
export async function cancelForfeit(matchId: Id, userId: Id): Promise<boolean> {
  const k = key(matchId, userId);
...
async function markDisconnected(matchId: Id, userId: Id): Promise<void> {
...
async function applyForfeit(matchId: Id, userId: Id): Promise<ForfeitResult> {
  return prisma.$transaction(async (tx) => {
    ...
    if (match.status !== 'open') return resolved;

    const participant = await tx.matchParticipant.findUnique({
      where: { matchId_userId: { matchId, userId } },
    });
    if (!participant) return resolved;

    const amount = toDecimal(participant.lockedAmount as unknown as Decimal);
    if (amount.lessThanOrEqualTo(0)) return resolved;
...
export function isAwaitingReconnect(matchId: Id, userId: Id): boolean {
  return pending.has(key(matchId, userId));
}

/** Clear every pending timer — used on shutdown and between tests. */
export function clearAllForfeitTimers(): void {
