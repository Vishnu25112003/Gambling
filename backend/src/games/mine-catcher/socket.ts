...
import type { BoardSize, MineCatcherState } from './types.js';
...
function startPlacementTimer(match: ActiveMatch): void {
...
function startTurnTimer(match: ActiveMatch, attackerId: string): void {
...
export function registerMineCatcherSocket(namespace: Namespace, socket: Socket): void {
  ...
  socket.on(MC_EVENTS.CREATE_MATCH, async (data: {
    ...
    try {
      const boardSize = (data.boardSize ?? 25) as BoardSize;
        ...
        socket.emit(MC_EVENTS.MATCH_STATE, {
          state: {
            ...
            boards: Object.fromEntries(
              Object.entries(activeMatch.state.boards).map(([k, v]) => [
                k,
                { ...v, mines: [...v.mines] },
              ]),
            ),
  ...
  socket.on(MC_EVENTS.PLACE_MINES, async (data: { cells?: number[] }) => {
    try {
  ...
  socket.on(MC_EVENTS.ATTACK_CELL, async (data: { cellIndex?: number }) => {
    try {
      const matchId = socketToMatch.get(socket.id);
      if (!matchId) {
        socket.emit(MC_EVENTS.ERROR, { message: 'Not in a match' });
  ...
  // --- REMATCH_REQUEST ---
  socket.on(MC_EVENTS.REMATCH_REQUEST, async (data: { matchId?: string }) => {
    try {
