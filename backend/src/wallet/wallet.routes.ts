...
walletRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!parsed.success) throw badRequest('An amount is required.');
...
walletRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!parsed.success) throw badRequest('A valid txSignature is required.');
    if (!isTreasuryConfigured()) throw serviceUnavailable('Treasury is not configured.');
