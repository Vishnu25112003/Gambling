...
import type { BallLogEntry, HandCricketState, InningsRecord } from './types.js';
...
export function createInitialState(
  ...
  const firstBatter = playerIds[Math.floor(Math.random() * 2)] as string;
  const firstBowler = playerIds.find((id) => id !== firstBatter) as string;
 ...
 */
export function startNewBall(state: HandCricketState, now = Date.now()): HandCricketState {
...
export function bothPicksIn(state: HandCricketState): boolean {
  if (!state.pendingBall || state.currentInningsIndex === null) return false;
  const innings = state.innings[state.currentInningsIndex];
  if (!innings) return false;
  return (
    state.pendingBall.picks[innings.batterId] !== undefined &&
...
export function resolveBall(
  state: HandCricketState,
): { state: HandCricketState; ballResult: BallResult } | null {
  if (!bothPicksIn(state) || state.currentInningsIndex === null) return null;

  const idx = state.currentInningsIndex;
  const innings = state.innings[idx];
  if (!innings || !state.pendingBall) return null;

  const batterPick = state.pendingBall.picks[innings.batterId] as number;
  const bowlerPick = state.pendingBall.picks[innings.bowlerId] as number;
  const out = batterPick === bowlerPick;
  const runsScored = out ? 0 : batterPick;
  const ballNumber = innings.ballsBowled + 1;

  const logEntry: BallLogEntry = { ball: ballNumber, batterPick, bowlerPick, runsScored, out };

  const newInnings: InningsRecord = {
    ...innings,
...
export function currentInnings(state: HandCricketState): InningsRecord | null {
  if (state.currentInningsIndex === null) return null;
 ...
 * Super-Over 3->4 swap.
 */
export function advanceInnings(state: HandCricketState): HandCricketState {
 ...
 */
export function compareRuns(state: HandCricketState): RunComparison {
  const n = state.innings.length;
  const a = state.innings[n - 2];
  const b = state.innings[n - 1];
  if (!a || !b) return { decided: false, winnerId: null, tied: true };

  if (a.runs === b.runs) return { decided: false, winnerId: null, tied: true };
 ...
 * the main match.
 */
export function startSuperOver(state: HandCricketState): HandCricketState {
  const playerIds = Object.keys(state.lives);
  const firstBatter = playerIds[Math.floor(Math.random() * playerIds.length)] as string;
  const firstBowler = playerIds.find((id) => id !== firstBatter) as string;
...
export interface MatchEndCheck {
  over: boolean;
  winnerId: string | null;
  endCause: HandCricketState['endCause'];
}
 ...
 */
export function checkMatchEnd(state: HandCricketState): MatchEndCheck {
  const n = state.innings.length;

  if (n === 2) {
    const cmp = compareRuns(state);
    if (cmp.decided) return { over: true, winnerId: cmp.winnerId, endCause: 'runs_higher' };
    return { over: false, winnerId: null, endCause: null };
  }

  if (n === 4) {
    const cmp = compareRuns(state);
    if (cmp.decided) return { over: true, winnerId: cmp.winnerId, endCause: 'super_over_decided' };
...
export function decrementLife(
  state: HandCricketState,
  userId: string,
): { state: HandCricketState; lifeLost: boolean; gameOver: boolean; winnerId: string | null } {
  ...
  const newState: HandCricketState = { ...state, lives: newLives };
...
export function markDisconnected(state: HandCricketState, userId: string): HandCricketState {
...
export function markReconnected(state: HandCricketState, userId: string): HandCricketState {
...
export function getOpponent(state: HandCricketState, userId: string): string | null {
